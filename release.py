...
def main():
    ...
    subprocess.run(['./macOS-x64/build-macos-x64.sh', 'femr', args.version, "1"], input=b'N', check=True)

    print('Uploading Intel Mac installer to S3. This may take a while...')

    s3.upload_file(
        f'./macOS-x64/target/pkg/femr-macos-installer-intel-{args.version}.pkg', 
        BUCKET_NAME, 
    ...
    # build the mac installer
    subprocess.run(['./macOS-x64/build-macos-x64.sh', 'femr', args.version, "2"], input=b'N', check=True)

    print('Uploading Arm Mac installer to S3. This may take a while...')

    s3.upload_file(
        f'./macOS-x64/target/pkg/femr-macos-installer-arm-{args.version}.pkg', 
        BUCKET_NAME, 
